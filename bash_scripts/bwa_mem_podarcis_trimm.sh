...
REF=/science/willerslev/users-shared/science-snm-willerslev-rlw363/ref_genome_PS/Psic_sn201_all.ph.fasta.gz
...
#bwa mem -M -t ${THD} -r "@RG\tID:${ID0}\tSM:${SM}\tCN:CGG\tPL:ILLUMINA\tLB:${LB}\tDS:maria.novosolov@sund.ku.dk" ${REF} ${R0}|samtools sort -m4G -@2 -o ${ONAM0}
#same as above but with the the real reads
bwa mem -M -t ${THD} -r "@RG\tID:${ID2}\tSM:${SM}\tCN:CGG\tPL:ILLUMINA\tLB:${LB}\tDS:maria.novosolov@sund.ku.dk" ${REF} ${R1} ${R2} |samtools sort -m4G -@2 -o ${ONAM}
